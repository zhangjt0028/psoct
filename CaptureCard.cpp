...
DataCapture::DataCapture(){
    ...
    AlazarSetCaptureClock(mainWidget::m_AlazarBoardHandle, EXTERNAL_CLOCK, SAMPLE_RATE_250MSPS, CLOCK_EDGE_RISING, 0);

    // 确定chnnal A 和 B的一些input要求
    AlazarInputControl(mainWidget::m_AlazarBoardHandle, CHANNEL_A, DC_COUPLING, INPUT_RANGE_PM_400_MV, IMPEDANCE_50_OHM);
    AlazarInputControl(mainWidget::m_AlazarBoardHandle, CHANNEL_B, DC_COUPLING, INPUT_RANGE_PM_400_MV, IMPEDANCE_50_OHM);


    // 确定触发模式，只使用external trigger
    AlazarSetTriggerOperation(mainWidget::m_AlazarBoardHandle, TRIG_ENGINE_OP_J, TRIG_ENGINE_J, TRIG_EXTERNAL, TRIGGER_SLOPE_POSITIVE,
        180, //TriggerLevelCode = 128 + 127 * TriggerLevelVolts / InputRangeVolts
        TRIG_ENGINE_K, TRIG_DISABLE, TRIGGER_SLOPE_POSITIVE, 128);
    AlazarSetExternalTrigger(mainWidget::m_AlazarBoardHandle, DC_COUPLING, ETR_TTL);
    ...
    AlazarSetTriggerTimeOut(mainWidget::m_AlazarBoardHandle, triggerTimeout_clocks);
    //retCode = AlazarConfigureAuxIO(mainWidget::m_AlazarBoardHandle, AUX_OUT_TRIGGER, 0);
    AlazarConfigureAuxIO(mainWidget::m_AlazarBoardHandle, AUX_IN_TRIGGER_ENABLE, TRIGGER_SLOPE_POSITIVE);
    ...
    AlazarGetChannelInfo(mainWidget::m_AlazarBoardHandle, &maxSamplesPerChannel, &bitsPerSample);
    ...
    AlazarBeforeAsyncRead(mainWidget::m_AlazarBoardHandle, CHANNEL_A, 0, // Must be 0
        m_samplesPerRecord, m_recordperbuffer, m_recordsPerAcquisition, m_admaFlags);


    for (U32 bufferIndex = 0; bufferIndex < mainWidget::BUFFER_COUNT; bufferIndex++/*, pBuffer += samplesPerBuffer*/)
    {
        AlazarPostAsyncBuffer(mainWidget::m_AlazarBoardHandle, m_volumeMemBuffer[bufferIndex], m_bytesPerBuffer);
    }
...
void DataCapture::EnableCC(){
    ...
    if (retCode == ApiSuccess)
    {
        for(;;)
        {
            ...
            {
                if (mainWidget::scanMode == 2 && m_buffersCompleted == TBD)
                {
                    retCode = AlazarAbortAsyncRead(mainWidget::m_AlazarBoardHandle);
                    return;
                }
                if (mainWidget::scanMode == 3 && m_buffersCompleted == TBD)
                {
                    retCode = AlazarAbortAsyncRead(mainWidget::m_AlazarBoardHandle);
                    return;
                }
                ...
                retCode = AlazarWaitAsyncBufferComplete(mainWidget::m_AlazarBoardHandle, pBuffer, m_timeout_ms);
                ...
                if (retCode == ApiSuccess)
                {
                   ...
                   // qDebug()<< m_curIndexInMemBuffer;
                    ...
                    //qDebug()<< index;
                    retCode = AlazarPostAsyncBuffer(mainWidget::m_AlazarBoardHandle, m_volumeMemBuffer[index], m_bytesPerBuffer);//
